    ...
    public class Startup
    {
        ...
        private static IBusControl CreateInMemQueue(IServiceProvider provider) =>
            Bus.Factory.CreateUsingInMemory(config =>
            {
                ...
                config.ReceiveEndpoint(ConfigConstant.QueueName, endPointConfig =>
                {
                    endPointConfig.ConfigureConsumer<OrderConsumer>(provider);

                    EndpointConvention.Map<ISendOrder>(endPointConfig.InputAddress);
                });
